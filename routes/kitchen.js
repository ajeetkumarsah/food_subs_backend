...
router.put('/:id', asyncHandler(async (req, res) => {
    try {
        ...
        uploadKitchen.single('img')(req, res, async function (err) {
            ...
            if (req.file) {
                image = `http://localhost:3000/image/kitchen/${req.file.filename}`;
            }
            ...
            try {
                const updatedKitchen = await Kitchen.findByIdAndUpdate(kitchenID, { name: name, image: image }, { new: true });
                if (!updatedKitchen) {
                    return res.status(404).json({ status: false, message: "Kitchen not found." });
                }
                res.json({ status: true, message: "Kitchen updated successfully.", data: null });
            } catch (error) {
                res.status(500).json({ status: false, message: error.message });
            }
